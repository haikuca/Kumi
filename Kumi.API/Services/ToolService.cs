...
namespace Kumi.API.Services
{
    ...
    public class ToolService(IToolCommandActions toolCommandActions, ToolAssembler toolAssembler)
    {
        ...
        public async Task<Result<ToolDto>> AddTool(ToolDto tool) 
        {
            return Result<ToolDto>.Success(
                toolAssembler.Assemble(
                    await toolCommandActions.AddTool(toolAssembler.Disassemble(tool))
                )
            );
        }
